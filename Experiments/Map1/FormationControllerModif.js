...
setInterval(() => {
  ...
  if (
    (Math.abs(
      controller.VS.Formation_Reference_Point[0] - setup.targetsPosition[0][0]
    ) <
      0.2) &
    (Math.abs(
      controller.VS.Formation_Reference_Point[1] - setup.targetsPosition[0][1]
    ) <
      0.2)
  ) {
    connectionTunnel.emit(QUAD1_COMMAND, {
